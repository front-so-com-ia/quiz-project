...
export function ResultsScreen() {
  ...
  const message = (score: number): string => {
    if (score >= 0 && score <= 200)
      return 'It’s past its prime... Take another stab at it';
    if (score > 200 && score <= 500)
      return 'Bang average.. Give it another shot';
    if (score > 500 && score <= 700)
      return "Great effort! You're clearly improving.";
    if (score > 700 && score <= 900)
      return "Incredible! You're among the top players.";
  ...
  return (
    <BaseLayout className="animate-pop-in text-center glass-card space-y-6 p-8">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white/5 rounded-xl p-4">
          <p className="text-3xl font-extrabold text-neon-purple">
            {hitRateAnswers}%
          </p>
          <p className="text-xs text-white/40 uppercase tracking-wider font-semibold mt-1">
            Hit Rate
